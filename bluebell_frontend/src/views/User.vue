  ...
  <div class="content">
    <div class="left">
      ...
      <div class="c-l-header">
        <div class="new btn-iconfont"
             :class="{ active: UserOrder }"
             @click="selectOrder('user')"
        >
          <i class="iconfont icon-polygonred"></i>你的发帖
        </div>
        <div class="top btn-iconfont"
             :class="{ active: CollectOrder }"
             @click="selectOrder('collect')"
        >
          <i class="iconfont icon-top"></i>收藏
        </div>
    ...
    <div class="right">
      <div class="communities">
        <h2 class="r-c-title">你的关注</h2>
        <ul class="r-c-content">
          <li class="r-c-item" v-for="user in userList" :key="user.user_id">
            <i class="icon"></i>
            <div @click="goOtherUser(user.user_id)">
        ...
        <div class="pagination">
          <el-pagination
              background
              layout="prev, pager, next"
              :total="this.concern_sum"
              :page-size="3"
              @prev-click="prevUserClick"
              @next-click="nextUserClick"
              :current-page="this.concernPage"
  ...
  methods: {
    ...
    getConcernList(){
      this.$axios({
        ...
        url:"/concern",
        params: {
          page: this.concernPage,
        }
      })
          .then(response => {
            console.log(response.data, 222);
            if (response.code == 1000) {
              this.userList = response.data.concern_user;
              this.concern_sum = response.data.concern_sum
